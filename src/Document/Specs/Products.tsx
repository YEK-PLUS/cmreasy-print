...
const ADRItem: FC<ADRItemProps> = ({label, text, value, mode, language}) => {
  return (
    <View style={styles.adrItemWrapper}>
      {label && (
        <IntlText id={label} mode={mode} language={language} direction="col" />
      )}
...
const ProductsFooter: FC<ProductsFooterProps> = ({data: _data}) => {
  const data = typeCheck(productsSchema, _data);
  const weight = data.reduce(
    (acc, {grossWeightInKg}) => acc + (grossWeightInKg || 0),
    0,
  );
...
const ProductsStatistics: FC<ProductsStatisticsProps> = ({
  data: _data,
  language,
  mode,
}) => {
    ...
    >
      <View style={styles.productsWrapper}>
        <View style={styles.titleWrapper}>
          ...
          <IntlText
            id="products.statisticalNumber"
            direction="col"
            mode={mode}
            language={language}
          />
        </View>
        {data.map(({statisticalNumber}, index) => (
          <ProductsItemValue center value={statisticalNumber} key={index} />
        ))}
      </View>
      <View style={styles.productsWrapper}>
        <View style={styles.titleWrapper}>
          ...
          <IntlText
            id="products.grossWeightInKg"
            direction="col"
            mode={mode}
            language={language}
          />
        </View>
        {data.map(({grossWeightInKg}, index) => (
          <ProductsItemValue
            center
            value={grossWeightInKg?.toString() || ''}
            key={index}
          />
        ))}
      </View>
      <View style={styles.productsWrapper}>
        <View style={styles.titleWrapper}>
          ...
          <IntlText
            id="products.volumeInM3"
            direction="col"
            mode={mode}
            language={language}
          />
        </View>
        {data.map(({volumeInM3}, index) => (
          <ProductsItemValue
            center
            value={volumeInM3?.toString() || ''}
            key={index}
          />
        ))}
      </View>
...
const ProductsItemValue: FC<ProductsItemValueProps> = ({
  value,
  center = false,
}) => {
  return (
...
const ProductsMain: FC<ProductsMainProps> = ({data, language, mode}) => {
  return (
    <View style={styles.productsMainWrapper}>
      <View style={styles.productsWrapper}>
        <View style={styles.titleWrapper}>
          <Text style={{...styles.titleOrder, color: ModeColor[mode]}}>6</Text>
          <IntlText
            id="products.marksAndNos"
            direction="col"
            mode={mode}
            language={language}
          />
        </View>
        {data.map(({marksAndNos}, index) => (
          <ProductsItemValue
            value={`${index + 1}. ${marksAndNos}`}
            key={index}
          />
        ))}
      </View>
      ...
      <View style={styles.productsWrapper}>
        <View style={styles.titleWrapper}>
          ...
          <IntlText
            id="products.numberOfPackages"
            direction="col"
            mode={mode}
            language={language}
          />
        </View>
        {data.map(({numberOfPackages}, index) => (
          <ProductsItemValue value={numberOfPackages} key={index} />
        ))}
      </View>
      ...
      <View style={styles.productsWrapper}>
        <View style={styles.titleWrapper}>
          ...
          <IntlText
            id="products.methodOfPacking"
            direction="col"
            mode={mode}
            language={language}
          />
        </View>
        {data.map(({methodOfPacking}, index) => (
          <ProductsItemValue value={methodOfPacking} key={index} />
        ))}
      </View>
      ...
      <View style={styles.productsWrapper}>
        <View style={styles.titleWrapper}>
          ...
          <IntlText
            id="products.natureOfTheGoods"
            direction="col"
            mode={mode}
            language={language}
          />
        </View>
        {data.map(({natureOfTheGoods}, index) => (
          <ProductsItemValue value={natureOfTheGoods} key={index} />
        ))}
      </View>
    </View>
  );
...
export const Products: FC<ProductsProps> = ({
  data: {products: _products, adr: _adr},
  language,
  mode,
}) => {
